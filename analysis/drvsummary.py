    ...
    for i in range(options.iterations):
        ...
        for vdata in rinfo_tree.getroot():
            ...
            if not pref in drvdata:
                drvdata[pref] = dict((i,{'drv':0,'tt':0,'xps':0,'z':0}) for i in range(options.iterations))
            ...
            drvdata[pref][i]['drv'] += 1
            vehnum += 1
            
        #saves the number of vehicles    
        #drvdata[pref][i]['drv'] = vehnum
#    for i in range(options.iterations):
...
#        print 'avg...', drvdata['0.5'][i]['tt']    
    ...
    for pref in outfiles:
        outfiles[pref].write((options.separator.join(['%s'] * 5) % ('#it','num_drv','z','tt','exp\n')).decode('string-escape'))
        
        for i in range(options.iterations):
        
            outfiles[pref].write((
                (options.separator.join(['%s'] * 5) + '\n') % (
                    i+1,
                    drvdata[pref][i]['drv'], 
                    drvdata[pref][i]['z'],
                    drvdata[pref][i]['tt'],
    ...
    print "Output files '%s_*.dvs' written." % os.path.join(options.path_to_input,options.output)
